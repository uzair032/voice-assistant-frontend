...
export default function App() {
  ...
  const connectWs = () => {
    return new Promise((resolve, reject) => {
      ...
      ws.onmessage = async (e) => {
        try {
          ...
          if (msg.type === "status") {
            ...
            setStatusMsg(msg.connected ? "Microphone ready." : "Disconnected.");
          } else if (msg.type === "reply") {
            setMessages((m) => [...m, { sender: "AI", text: msg.text }]);
            if (msg.audio_b64) {
              const audioBuf = base64ToArrayBuffer(msg.audio_b64);
              await playAudioBuffer(audioBuf);
            }
          } else if (msg.type === "summary") {
            const conv = msg.conversation.map((c) => ({
              sender: c.role === "assistant" ? "AI" : "User",
              text: c.content,
            }));
            setMessages(conv);
            setStatusMsg("Conversation summary received.");
          }
        } catch (err) {
      ...
      ws.onclose = () => {
        setConnected(false);
        setStatusMsg("Disconnected from backend.");
      };
  ...
  const startRecording = async () => {
    if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) await connectWs();
    const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
    streamRef.current = stream;
  ...
  return (
    <div style={{ padding: 24, fontFamily: "Arial, sans-serif" }}>
      <h2>🎙️ Real-time Voice Assistant</h2>
      <div style={{ marginBottom: 12 }}>
        <button onClick={connected ? stopRecording : startRecording} style={{ padding: "8px 12px" }}>
        ...
        <span style={{ marginLeft: 10 }}>
          ...
          {recording && <span style={{ marginLeft: 8, color: "orange" }}>• Recording</span>}
        </span>
      </div>
      <div style={{ marginBottom: 10, color: "#555" }}>{statusMsg}</div>
      <div style={{ background: "#f7f7f7", borderRadius: 8, padding: 12, height: 340, overflowY: "auto" }}>
        {messages.length === 0 ? (
          <div>No messages yet.</div>
        ) : (
          messages.map((msg, i) => (
            <div key={i} style={{ marginBottom: 10 }}>
              <b style={{ color: msg.sender === "AI" ? "#007bff" : "#000" }}>{msg.sender}:</b>{" "}
              {msg.text}
            </div>
          ))
        )}
